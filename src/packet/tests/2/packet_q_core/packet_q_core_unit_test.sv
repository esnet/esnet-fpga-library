...
module packet_q_core_unit_test #(
    ...
    // Parameters
    //===================================
    localparam int  INPUT_SEL_WID = NUM_INPUT_IFS > 1 ? $clog2(NUM_INPUT_IFS) : 1;
    localparam int  OUTPUT_SEL_WID = NUM_OUTPUT_IFS > 1 ? $clog2(NUM_OUTPUT_IFS) : 1;

    localparam int  NUM_QS = 8;
    ...
    localparam int  Q_DEPTH = MEM_DEPTH;
    ...
    logic [OUTPUT_SEL_WID-1:0] packet_in_dest [NUM_INPUT_IFS];
    logic [Q_SEL_WID-1:0]      packet_in_q    [NUM_INPUT_IFS];
    packet_intf #(.DATA_BYTE_WID(DATA_IN_BYTE_WID), .META_WID(META_WID)) packet_in_if [NUM_INPUT_IFS]   (.clk);

    mem_wr_intf #(.DATA_WID(MEM_DATA_WID), .ADDR_WID(PTR_WID))  packet_desc_mem_wr_if (.clk);
    mem_wr_intf #(.DATA_WID(DATA_IN_WID),  .ADDR_WID(ADDR_WID)) packet_data_mem_wr_if [NUM_INPUT_IFS] (.clk);
    mem_wr_intf #(.DATA_WID(MEM_DATA_WID), .ADDR_WID(ADDR_WID)) __mem_wr_if [NUM_MEM_DATA_IFS] (.clk);

    logic                      enq_ack  [NUM_OUTPUT_IFS];
    logic                      enq_nack [NUM_OUTPUT_IFS];
    logic [INPUT_SEL_WID-1:0]  enq_src  [NUM_OUTPUT_IFS];
    logic [Q_SEL_WID-1:0]      enq_q    [NUM_OUTPUT_IFS];
    logic [PKT_SIZE_WID-1:0]   enq_size [NUM_OUTPUT_IFS];

    logic                      deq_req  [NUM_OUTPUT_IFS];
    logic                      deq_rdy  [NUM_OUTPUT_IFS];
    logic [Q_SEL_WID-1:0]      deq_q    [NUM_OUTPUT_IFS];
    logic                      deq_ack  [NUM_OUTPUT_IFS];
    logic                      deq_nack [NUM_OUTPUT_IFS];
    logic [OUTPUT_SEL_WID-1:0] deq_src  [NUM_OUTPUT_IFS];
    logic [PKT_SIZE_WID-1:0]   deq_size [NUM_OUTPUT_IFS];

    mem_wr_intf #(.DATA_WID(MEM_DATA_WID), .ADDR_WID(PTR_WID)) q_mem_wr_if [NUM_OUTPUT_IFS] (.clk);
    mem_rd_intf #(.DATA_WID(MEM_DATA_WID), .ADDR_WID(PTR_WID)) q_mem_rd_if [NUM_OUTPUT_IFS] (.clk);

    mem_rd_intf #(.DATA_WID(MEM_DATA_WID), .ADDR_WID(PTR_WID))  packet_desc_mem_rd_if (.clk);
    mem_rd_intf #(.DATA_WID(DATA_OUT_WID), .ADDR_WID(ADDR_WID)) packet_data_mem_rd_if [NUM_OUTPUT_IFS] (.clk);
    ...
    logic [INPUT_SEL_WID-1:0] packet_out_src [NUM_OUTPUT_IFS];
    logic [Q_SEL_WID-1:0]     packet_out_q   [NUM_OUTPUT_IFS];
    ...
    // Memory
    //===================================
    localparam int NUM_MEM_CHANNELS = NUM_MEM_DATA_IFS + 1 + NUM_OUTPUT_IFS;
    ...
    for (genvar g_if = 0; g_if < NUM_INPUT_IFS; g_if++) begin : g__input_if
        ...
        mem_wr_aggregate #(.N(NUM_MEM_WR_IFS_PER_INPUT)) i_mem_wr_aggregate (.from_controller(packet_data_mem_wr_if[g_if]), .to_peripheral (__mem_wr_if__slice));
    ...
    for (genvar g_if = 0; g_if < NUM_OUTPUT_IFS; g_if++) begin : g__output_if
        ...
        // Distribute (wide) memory interfaces over multiple (narrow) interfaces (adapt 512-bit AXI-S interface to 256-bit AXI-3 HBM interface, for example)
        mem_rd_aggregate #(.N(NUM_MEM_RD_IFS_PER_OUTPUT)) i_mem_rd_aggregate (.from_controller(packet_data_mem_rd_if[g_if]), .to_peripheral (__mem_rd_if__slice));
        ...
        axi3_from_mem_adapter #(
            ...
            .BURST_SUPPORT ( 0 ),
            .WR_ID ( NUM_MEM_DATA_IFS * 2 + 1 + g_if ),
            .RD_ID ( NUM_MEM_DATA_IFS * 2 + 1 + g_if )
        ) i_axi3_from_mem_adapter (
            .clk,
            .srst,
            .init_done (),
            .mem_wr_if ( q_mem_wr_if [g_if] ),
            .mem_rd_if ( q_mem_rd_if [g_if] ),
            .axi3_if   ( axi3_if[NUM_MEM_DATA_IFS + 1 + g_if] )
        );
    ...
    generate
        int cnt = 0;
        for (genvar g_if = 0; g_if < NUM_OUTPUT_IFS; g_if++) begin : g__q_controller
            int cnt = 0;
            bit busy = 0;
            always @(posedge clk) begin
                if (srst) cnt <= 0;
                else begin
                    if      (enq_ack[g_if] && !deq_ack[g_if]) cnt <= cnt + 1;
                    else if (deq_ack[g_if] && !enq_ack[g_if]) cnt <= cnt - 1;
                end
            end
            assign deq_q[g_if] = 0;
            assign deq_req[g_if] = cnt > 0;

            always @(posedge clk) begin
                if (srst) busy <= 0;
                else if (deq_req[g_if] && deq_rdy[g_if]) busy <= 1'b1;
                else if (deq_ack[g_if] || deq_nack[g_if]) busy <= 1'b0;
            end
    ...
    task setup();
        ...
        for (int i=0; i<NUM_INPUT_IFS;i++) begin
            packet_in_q[i] = 0;
            packet_in_dest[i] = 0;
        end
