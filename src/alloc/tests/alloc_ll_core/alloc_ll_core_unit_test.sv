...
module alloc_ll_core_unit_test #(
    ...
    string name = $sformatf("alloc_ll_core_%0dl_%0db_ut", LIST_CONTEXTS, PTR_WID);
    ...
    mem_ram_sdp #(
        .SPEC ('{ADDR_WID: PTR_WID, DATA_WID: 256, ASYNC: 0, RESET_FSM: 0, OPT_MODE: mem_pkg::OPT_MODE_DEFAULT})
    ) i_mem_ram_sdp (
        .*
    );
...
endmodule : alloc_ll_core_unit_test

// 'Boilerplate' unit test wrapper code
//  Builds unit test for a specific configuration in a way
//  that maintains SVUnit compatibility
`define ALLOC_LL_CORE_UNIT_TEST(LIST_CONTEXTS,PTR_WID)\
  import svunit_pkg::svunit_testcase;\
  svunit_testcase svunit_ut;\
  alloc_ll_core_unit_test#(LIST_CONTEXTS,PTR_WID) test();\
  function void build();\
    test.build();\
...
module alloc_ll_core_16l_8b_unit_test;
`ALLOC_LL_CORE_UNIT_TEST(16,8);
endmodule
/*
// (Distributed RAM) 9-bit (2 slices) pointer allocator
module alloc_ll_core_9b_2s_unit_test;
`ALLOC_LL_CORE_UNIT_TEST(9,2);
endmodule

// (Block RAM) 4096-entry, 12-bit pointer allocator
module alloc_ll_core_12b_unit_test;
`ALLOC_LL_CORE_UNIT_TEST(12,1);
endmodule

// (Block RAM) 4096-entry, 13-bit (2 slices) pointer allocator
module alloc_ll_core_13b_2s_unit_test;
`ALLOC_LL_CORE_UNIT_TEST(13,2);
endmodule

// (Block RAM) 65536-entry, 16-bit pointer allocator
module alloc_ll_core_16b_unit_test;
`ALLOC_LL_CORE_UNIT_TEST(16,1);
endmodule

// (Ultra RAM) 262144-entry, 18-bit pointer allocator
module alloc_ll_core_18b_unit_test;
`ALLOC_LL_CORE_UNIT_TEST(18,1);
endmodule

// (Ultra RAM) 262144-entry, 18-bit pointer allocator
module alloc_ll_core_18b_2s_unit_test;
`ALLOC_LL_CORE_UNIT_TEST(18,2);
endmodule
