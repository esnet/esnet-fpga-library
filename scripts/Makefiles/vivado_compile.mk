...
SYNTH_SOURCES_TMP = $(COMPONENT_OUT_SYNTH_PATH)/sources.tcl.tmp
...
$(SYNTH_SOURCES_OBJ): $(SRCS) $(HDRS) Makefile $(SUBCOMPONENT_SYNTH_OBJS) | $(COMPONENT_OUT_SYNTH_PATH)
	...
	@echo "#" >> $(SYNTH_SOURCES_TMP)
	@echo "# NOTE: This file is autogenerated. DO NOT EDIT." >> $(SYNTH_SOURCES_TMP)
	@echo "# =====================================================" >> $(SYNTH_SOURCES_TMP)
	@echo >> $(SYNTH_SOURCES_TMP)
	@echo "# Defines" >> $(SYNTH_SOURCES_TMP)
	@echo "# ------------------------" >> $(SYNTH_SOURCES_TMP)
	@if [ ! -z "$(strip $(SYNTH_DEFINES))" ]; then \
		echo "set verilog_defines [get_property verilog_define [current_fileset]]" >> $(SYNTH_SOURCES_TMP); \
		echo "lappend verilog_defines {*}{" >> $(SYNTH_SOURCES_TMP); \
	fi
	@-for define in $(SYNTH_DEFINES); do \
		echo $$define >> $(COMPONENT_OUT_SYNTH_PATH)/defines.f; \
		echo "\t$$define" >> $(SYNTH_SOURCES_TMP); \
	done
	@if [ ! -z "$(strip $(SYNTH_DEFINES))" ]; then \
		echo "}" >> $(SYNTH_SOURCES_TMP); \
		echo "set_property verilog_define \$$verilog_defines [current_fileset]" >> $(SYNTH_SOURCES_TMP); \
	fi
	@echo >> $(SYNTH_SOURCES_TMP)
	@echo "# Xilinx BD source listing" >> $(SYNTH_SOURCES_TMP)
	@echo "# ------------------------" >> $(SYNTH_SOURCES_TMP)
	@-for bdfile in $(abspath $(SYNTH_BD_FILES)); do \
		echo $$bdfile >> $(COMPONENT_OUT_SYNTH_PATH)/bd_srcs.f; \
		echo "read_bd -quiet $$bdfile" >> $(SYNTH_SOURCES_TMP); \
	done
	@echo >> $(SYNTH_SOURCES_TMP)
	@echo "# Xilinx IP source listing" >> $(SYNTH_SOURCES_TMP)
	@echo "# ------------------------" >> $(SYNTH_SOURCES_TMP)
	@if [ ! -z "$(strip $(SYNTH_IP_XCI_FILES))" ]; then \
		echo "read_ip -quiet {" >> $(SYNTH_SOURCES_TMP); \
	fi
	@-for xcifile in $(abspath $(SYNTH_IP_XCI_FILES)); do \
		echo $$xcifile >> $(COMPONENT_OUT_SYNTH_PATH)/ip_srcs.f; \
		echo "\t$$xcifile" >> $(SYNTH_SOURCES_TMP); \
	done
	@if [ ! -z "$(strip $(SYNTH_IP_XCI_FILES))" ]; then \
		echo "}" >> $(SYNTH_SOURCES_TMP); \
	fi
	@echo >> $(SYNTH_SOURCES_TMP)
	@echo "# Verilog source file listing" >> $(SYNTH_SOURCES_TMP)
	@echo "# ---------------------------" >> $(SYNTH_SOURCES_TMP)
	@if [ ! -z "$(strip $(SYNTH_V_SRC_FILES))" ]; then \
		echo "add_file -quiet {" >> $(SYNTH_SOURCES_TMP); \
	fi
	@-for vsrcfile in $(abspath $(SYNTH_V_SRC_FILES)); do \
		echo $$vsrcfile >> $(COMPONENT_OUT_SYNTH_PATH)/v_srcs.f; \
	    echo "\t$$vsrcfile" >> $(SYNTH_SOURCES_TMP); \
	done
	@if [ ! -z "$(strip $(SYNTH_V_SRC_FILES))" ]; then \
		echo "}" >> $(SYNTH_SOURCES_TMP); \
	fi
	@echo >> $(SYNTH_SOURCES_TMP)
	@echo "# Verilog header file listing" >> $(SYNTH_SOURCES_TMP)
	@echo "# ---------------------------" >> $(SYNTH_SOURCES_TMP)
	...
	@-for vhdrfile in $(abspath $(SYNTH_V_HDR_FILES)); do \
		...
		echo "\t$$vhdrfile" >> $(SYNTH_SOURCES_TMP); \
	done
	@if [ ! -z "$(strip $(SYNTH_V_HDR_FILES))" ]; then \
		echo "}" >> $(SYNTH_SOURCES_TMP); \
	fi
	@echo >> $(SYNTH_SOURCES_TMP)
	@echo "# SystemVerilog package listing" >> $(SYNTH_SOURCES_TMP)
	@echo "# -----------------------------" >> $(SYNTH_SOURCES_TMP)
	...
	@-for svpkgfile in $(SYNTH_SV_PKG_FILES); do \
		...
		echo "\t$$svpkgfile" >> $(SYNTH_SOURCES_TMP); \
	done
	@if [ ! -z "$(strip $(SYNTH_SV_PKG_FILES))" ]; then \
		echo "}" >> $(SYNTH_SOURCES_TMP); \
	fi
	@echo >> $(SYNTH_SOURCES_TMP)
	@echo "# SystemVerilog source file listing" >> $(SYNTH_SOURCES_TMP)
	@echo "# ---------------------------------" >> $(SYNTH_SOURCES_TMP)
	...
	@-for svsrcfile in $(abspath $(SYNTH_SV_SRC_FILES)); do \
	    ...
	    echo "\t$$svsrcfile" >> $(SYNTH_SOURCES_TMP); \
	done
	@if [ ! -z "$(strip $(SYNTH_SV_SRC_FILES))" ]; then \
		echo "}" >> $(SYNTH_SOURCES_TMP); \
	fi
	@echo >> $(SYNTH_SOURCES_TMP)
	@echo "# SystemVerilog header file listing" >> $(SYNTH_SOURCES_TMP)
	@echo "# ---------------------------------" >> $(SYNTH_SOURCES_TMP)
	...
	@-for svhdrfile in $(abspath $(SYNTH_SV_HDR_FILES)); do \
		...
		echo "\t$$svhdrfile" >> $(SYNTH_SOURCES_TMP); \
	done
	@if [ ! -z "$(strip $(SYNTH_SV_HDR_FILES))" ]; then \
		echo "}" >> $(SYNTH_SOURCES_TMP); \
	fi
	...
	@echo >> $(SYNTH_SOURCES_TMP)
	@echo "# Synthesized DCP source listing" >> $(SYNTH_SOURCES_TMP)
	@echo "# ------------------------------" >> $(SYNTH_SOURCES_TMP)
	@if [ ! -z "$(strip $(SYNTH_DCP_FILES))" ]; then \
		echo "add_files -quiet -norecurse {" >> $(SYNTH_SOURCES_TMP); \
	fi
	@-for dcpfile in $(abspath $(SYNTH_DCP_FILES)); do \
		echo $$dcpfile >> $(COMPONENT_OUT_SYNTH_PATH)/dcp_srcs.f; \
		echo "\t$$dcpfile" >> $(SYNTH_SOURCES_TMP); \
	done
	@if [ ! -z "$(strip $(SYNTH_DCP_FILES))" ]; then \
		echo "}" >> $(SYNTH_SOURCES_TMP); \
	fi
